...
class controller extends Module {
  ...
  io.flush_en := io.flush_apply(4).asBools
  // io.flush_en := MuxCase(
  //     io.flush_apply(4).asBools,
  //     List(
  //       (io.flush_apply(4).asUInt =/= 0.U) -> io.flush_apply(4).asBools,
  //       (io.flush_apply(3).asUInt =/= 0.U) -> io.flush_apply(3).asBools,
  //       (io.flush_apply(2).asUInt =/= 0.U) -> io.flush_apply(2).asBools,
  //       (io.flush_apply(1).asUInt =/= 0.U) -> io.flush_apply(1).asBools,
  //       (io.flush_apply(0).asUInt =/= 0.U) -> io.flush_apply(0).asBools,
  //     ),
  //   )
