...
class IW_IO extends Bundle with Parameters {
    ...
    val ms_to_ws_bus = Input(new ms_to_ws_bus) 
...
class IW extends Module with Parameters {
    ...
    val ms_to_ws_bus_r = RegInit(0.U.asTypeOf(new ms_to_ws_bus))
    ...
    val ws_final_result = ms_to_ws_bus_r.final_result
    // val ws_pc           = ms_to_ws_bus_r(31, 0)
    val ws_pc = ms_to_ws_bus_r.pc

    val ms_res_from_mem = ms_to_ws_bus_r.ms_res_from_mem.asBool
